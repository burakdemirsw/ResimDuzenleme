...
namespace ResimDuzenleme
{
    public partial class NebimSilmeIslemleri : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void DeleteInvoice(string InvoiceID, string XML, bool FaturaSil, bool IrsaliyeSil, bool SiparisSil)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = new SqlCommand("MSG_DeleteTPCancelInvoiceID", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("@InvoiceHeaderID", InvoiceID);
                    command.Parameters.AddWithValue("@XML", XML);
                    command.Parameters.AddWithValue("@FaturaSil", FaturaSil);
                    command.Parameters.AddWithValue("@IrsaliyeSil", IrsaliyeSil);
                    command.Parameters.AddWithValue("@SiparisSil", SiparisSil);
        ...
        [Test]
        public async void CancelEArchiveInvoice(string InvoiceID, string InvoiceUUID, string IptalNotu)
        {
            ...
            {
                FATURA_ID = InvoiceID,//iptal edilecek faturanın ıd'si ve uuid'si yazılmalıdır.
                FATURA_UUID = InvoiceUUID,
                IPTAL_NOTU = IptalNotu
            };
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    using (SqlCommand command = new SqlCommand("MSG_GetEInvoiceNumber", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("@InvoiceHeaderID", textBox1.Text);

                        connection.Open();
                        // ExecuteReader kullanarak sorgudan dönen sonuçları alıyoruz
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                string eInvoiceNumber = reader["EInvoiceNumber"].ToString();
                                string invoiceHeaderID = reader["InvoiceHeaderID"].ToString();

                                // Belirttiğiniz fonksiyonu çağırabilirsiniz
                                CancelEArchiveInvoice(eInvoiceNumber, invoiceHeaderID, "İptal");
                            }
                        }
                    }
                }
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    using (SqlCommand command = new SqlCommand("MSG_DeleteTPCancelInvoiceID", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("@InvoiceHeaderID", textBox1.Text);
                        command.Parameters.AddWithValue("@XML", htmlContent);
                        command.Parameters.AddWithValue("@FaturaSil", faturaSil);
                        command.Parameters.AddWithValue("@IrsaliyeSil", irsaliyeSil);
                        command.Parameters.AddWithValue("@SiparisSil", siparisSil);
        ...
        private void NebimSilmeIslemleri_Load(object sender, EventArgs e)
        {
            string storeCode = Properties.Settings.Default.StoreCode;
